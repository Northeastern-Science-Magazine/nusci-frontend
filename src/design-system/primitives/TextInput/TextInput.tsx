...
export const TextInput = (props: TextInputProps) => {
  ...
  const multiline =
    props.multiline ??
    ('rows' in props && props.rows !== undefined ? true : false);

  const baseClassName = clsx(
    textInputVariantsCN({ variant, size, color, ...restProps }),
    className,
  );
  ...
  return (
    <div className="flex flex-col items-start">
      ...
      {multiline ? (
        (() => {
          const textareaProps = props as Extract<
            TextInputProps,
            { multiline?: true }
          > & { rows?: number };
          const handleKeyDown = (
          ...
          return (
            <textarea
              ...
              className={clsx(
                baseClassName,
                textareaProps.resize ? 'resize-y' : 'resize-none min-h-0',
              )}
              rows={textareaProps.rows ?? 1}
              style={{
                maxHeight: textareaProps.rows === 1 ? '2.5rem' : undefined,
              }}
